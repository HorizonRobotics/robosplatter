...
def get_header_guard_dmlc(filename):
    ...
    start_folder = ["module", "include"]
    for f in start_folder:
        idx = file_path_from_root.find(f)
        if idx != -1:
            file_path_from_root = file_path_from_root[idx:]
            break

    # prefix = _HELPER.project_name + "_"
    project_prefix = _HELPER.project_name + "_"
    prefix = ""
    is_in_module = file_path_from_root.find("module") != -1
    if is_in_module:
    ...
    for f in folder:
        ...
        if idx != -1:
            if is_in_module:
                prefix += file_path_from_root[0:idx]
            offset = idx + len(f)
            file_path_from_root = prefix + file_path_from_root[offset:]
            break

    if not file_path_from_root.startswith(project_prefix[0:-1]):
        file_path_from_root = project_prefix + file_path_from_root

    ret = re.sub(r"[-./\s]", "_", file_path_from_root).upper() + "_"
